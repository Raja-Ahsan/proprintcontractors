...
export default function Edit({ product, categories }) {
    ...
    /** @type {string[]} */
    const existingGalleryUrls = Array.isArray(product.gallery_urls)
    ...
    function submit(e) {
        ...
        let varsPayload = JSON.parse(form.data.variations_json || '[]');
        if (Array.isArray(varsPayload)) {
            varsPayload = varsPayload.map((row) => {
        ...
        form.transform((data) => {
            const next = {
                ...data,
                _method: 'put',
                attribute_defs_json: JSON.stringify(defsPayload),
                variations_json: JSON.stringify(varsPayload),
            };
            ...
            if (form.data.type === 'variable') {
                ...
                next.gallery_remove_indexes = galleryRemoveIndexes;
                    ...
                    {!isVariable && (
                        <>
                            ...
                            {existingGalleryUrls.length > 0 && (
                                <div>
                                    <InputLabel value="Current gallery" />
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        Check &quot;Remove&quot; to delete when you
                                        save.
                                    </p>
                                    <div className="mt-3 flex flex-wrap gap-3">
                                        {existingGalleryUrls.map((src, idx) => (
                                            <div
                                                key={src}
                                                className="relative rounded-lg border border-border p-1"
                                            >
                                                <img
                                                    src={src}
                                                    alt=""
                                                    className={`h-16 w-16 rounded-md object-cover ${galleryRemoveIndexes.includes(idx) ? 'opacity-40 grayscale' : ''}`}
                                                />
                                                <label className="mt-2 flex items-center gap-1 text-[11px] text-foreground">
                                                    <input
                                                        type="checkbox"
                                                        checked={galleryRemoveIndexes.includes(
                                                            idx,
                                                        )}
                                                        onChange={() =>
                                                            toggleGalleryRemove(idx)
                                                        }
                            ...
                            <div>
                                <InputLabel
                                    htmlFor="gallery_images_edit"
                                    value="Add gallery images (optional)"
                                />
                                <input
                                    id="gallery_images_edit"
                                    type="file"
                                    multiple
                                    accept="image/*"
                                    onChange={(e) => {
                                ...
                                />
                                <p className="mt-1 text-xs text-muted-foreground">
                                    Up to 20 total; removals above free space for new
                                    uploads.
                                </p>
                                ...
                                <InputError
                                    message={
                                        ...
                                        form.errors.gallery_remove_indexes
                                    }
                                    className="mt-2"
                                />
                            </div>
